...
def tile_pair(
    ...
    tile_size: int,
    overlap: int,
    raw_foreground: set[int],
    max_background_fraction: float,
) -> int:
    """Write paired tiles; use mask CRS/transform because source files omit them."""
    ...
    with rasterio.open(pair.image) as image, rasterio.open(pair.mask) as mask:
        if image.shape != mask.shape:
            raise ValueError(f"크기 불일치: {pair.image} / {pair.mask}")
        for row in _origins(image.height, tile_size, tile_size - overlap):
            for col in _origins(image.width, tile_size, tile_size - overlap):
                ...
                mask_tile = mask.read(1, window=window, boundless=True, fill_value=0)
                foreground_fraction = np.isin(mask_tile, list(raw_foreground)).mean()
                ...
                image_profile.update(driver="GTiff", width=tile_size, height=tile_size, count=len(channels), crs=mask.crs, transform=transform(window, mask.transform))
                mask_profile = mask.profile.copy()
                mask_profile.update(driver="GTiff", width=tile_size, height=tile_size, count=1, transform=transform(window, mask.transform))
...
def main() -> None:
    ...
    pairs = discover_pairs(dataset["root_dir"], dataset["train"])
    ...
    if args.tile_output:
        count = sum(tile_pair(pair, Path(args.tile_output), channels, int(dataset["tile_size"]), args.overlap, set(int(key) for key in dataset["raw_class_map"]), args.max_background_fraction) for pair in pairs)
